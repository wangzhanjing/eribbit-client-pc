<template>
  <div class="xtx-cart-page">
    <div class="container">
      ...
      <div class="cart">
        <table>
          <thead>
            <tr>
              <th width="120">
                <XtxCheckbox @change="checkedAll"
                             :modelValue="$store.getters['cart/isCheckedAll']">全选</XtxCheckbox>
              </th>
          ...
          <tbody>
            ...
            <tr v-else
                ...
                <div class="goods">
                  ...
                  <div>
                    ...
                    <CartSku :attrsText="item.attrsText"
                             :skuId="item.skuId"
                             @changeSku=" ($event )=> changeSku(item.skuId,$event)"></CartSku>
                  </div>
                </div>
              </td>
              <td class="tc">
              ...
              <td class="tc">
                <XtxNumbox @change="$event=> updateItemCount(item.skuId,$event)"
              ...
              <td class="tc">
                ...
                <p><a class="green"
                     href="javascript:;"
                     @click="deleteItem(item.skuId)">删除</a></p>
          ...
          <tbody v-if="$store.getters['cart/invalidList'].length>0">
            ...
            <tr v-for="item in $store.getters['cart/validList']"
                :key="item.skuId">
              <td>
      ...
      <div class="action">
        <div class="batch">
          <XtxCheckbox :modelValue="$store.getters['cart/isCheckedAll']"
                       @change="checkedAll">全选</XtxCheckbox>
          <a href="javascript:;"
             @click="batchDeleteCartItem">删除商品</a>
...
import confirm from '@/components/library/Confrim'
...
export default {
  ...
  setup () {
    ...
    // 批量删除
    const batchDeleteCartItem = () => {
      confirm({ text: '你确认全部删除嘛' }).then(() => {
        store.dispatch('cart/batchDeleteCartItem', true)
        Message({ type: 'success', text: '删除成功' })
      }).catch(() => {
        console.log(0)
      })
    ...
    // 修改单个商品数量
    const updateItemCount = (skuId, count) => {
